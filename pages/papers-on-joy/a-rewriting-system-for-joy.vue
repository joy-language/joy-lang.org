<template>
  <article>
    ...
    <p>
      A <em>rewriting system</em> consists of a set of syntactic rules for performing
      ...
      binary rewriting relation will be written in infix notation as <code>=&gt;</code>,
      pronounced "can be rewritten as". The following are some sample rules for the <kbd>+</kbd>  operator, the <kbd>&lt;</kbd> predicate and the <kbd>dip</kbd> combinator.
    </p>
    <pre>        2  3  +   =&gt;   5
            2  3  &lt;   =&gt;   true
            a  [P]  dip   =&gt;   P  a
    ...
    In the last example,
    <code>P</code> is any program and <code>a</code> is any literal (such as a number)
    ...
    and the operator or <em>constructor</em> is <code>+</code>, so they all refer to
    ...
    second one in the last line. That would treat the short program <code>2 3 +</code>as being composed of two operands and one operator or constructor. It would also
    treat the gap between <code>2</code> and <code>3</code> as quite different from the
    gap between <code>3</code> and <code>+</code>. The difference would be explained
    away as a syntactic coincidence due to the choice of notation. Apart from <code>+</code>there would be very many term constructors.
    <p>
      However, Joy has operators for manipulating the top few elements of the stack, such
      as <kbd>swap</kbd>, <kbd>dup</kbd> and <kbd>pop</kbd>. These are also found in
      the language <em>Forth</em>, see for example \AX{Salman {\it et al}}{1984}{Salman-etal:84}
      ...
      to be of the same type. For example, the following is a rewrite rule for <kbd>swap</kbd>:
    </p>
    <pre>        a  b  swap   =&gt;   b  a
    </pre>
    ...
    They all eventually have to reduce to <code>5</code>, just like the earlier
    Joy program <code>2 3 +</code>. It suggests that in the latter the gaps have to be
    ...
    system. Such a system is equivalent to a term rewriting system with a <em>concatenation constructor</em>for programs as the only constructor. To handle combinators, a <em>quotation constructor</em>has to be introduced as a second constructor.
    <p>
    ...
    <h1>String and term rewriting systems</h1>
    <p>
      Rewriting systems can be classified according to the entities that are being rewritten.
      In a <em>string rewriting system</em> those entities are linear sequences of
      symbols or strings of symbols. In a <em>term rewriting system</em> these entities
      are expressions or terms build from operands and operators. In a <em>graph rewriting system</em>  they are graphs of various kinds.
    </p>
    <p>
      A <em>string rewriting system</em> is based on an <em>alphabet</em> which is just
    ...
    and a relation <kbd>=&gt;</kbd>, a set of such pairs or rules. A wider relation <kbd>==&gt;</kbd>is defined as follows: For strings <code>w</code>, <code>x</code>, <code>y</code>and <code>z</code>,
    <pre>        w x z   ==&gt;   w y z
    </pre>
    if and only if <code>x =&gt; z</code>. This relation allows replacement of
    <code>x</code> by <code>y</code> in arbitrary contexts <code>w..z</code>.
    <p>
      A <em>term rewriting system</em> also requires an alphabet of symbols. Each symbol
      has an associated <em>arity</em> which is a natural number (0, 1, 2, ...). Symbols
      of arity 0 are nullary symbols or operands, symbols of positive arity are unary,
      binary, ternary and so on symbols or operators. A <em>term</em> over such an alphabet
    ...
    where <code>x</code> and <code>y</code> now have to be terms. A term rewriting
    system consists of an alphabet of symbols, each with their own arity, and a set of
    such rules. A wider relation <kbd>==&gt;</kbd> is defined as follows:
    <pre>        y   ==&gt;   z
    </pre>
    if and only if <code>y</code> and <code>z</code> are alike terms except that
    <code>y</code> contains a subterm <code>u</code> where <code>z</code> contains a
    subterm <code>w</code> such that <code>u =&gt; w</code>.
    <p>
      In any rewriting system it is useful to define <kbd>==&gt;&gt;</kbd> as the reflexive
      transitive closure of <code> ==&gt; </code>.
    </p>
    <pre>        x   ==&gt;&gt;   z
    </pre>
    ...
    In the second line the subexpression <code>2 3 +</code> on the left is called
    a reducible expression or <em>redex</em> since it can be reduced using the rule in
    the first line.
    <p>
      ...
      symbol, say <code>s</code> from the string alphabet is interpreted as a unary
      operator <em> append</em> <code>s</code>. The appending is either uniformly on
    ...
    <p>
      ...
      the unary <em>quotation constructor</em> which is needed for the combinators.
    </p>
    <p>
      Rewriting systems can be based just on unconditional rules of the form <code>x =&gt; y</code>,
      but they can also have <em>conditional rule</em>s. Such rules state that certain
      rewritings are permitted provided certain other rewritings are permitted. The next
      sections give a <em>conditional rewriting system</em> for Joy.
    </p>
    <p>
    ...
    <h1>A conditional rewriting system for Joy</h1>
    <p>
    ...
    <p>
      ...
      <em>program concatenation</em> and <em>program quotation</em>. The system to be
      presented here uses (unconditional) axioms of the form
    </p>
    <pre>        P   =&gt;   Q
    </pre>
    where <code>P</code> and <code>Q</code> are programs. There are also conditional
    rules of the following forms, where <code>R</code>, <code>S</code>, <code>T</code>and <code>U</code> are further programs.
    <pre>        P  =&gt;  Q   :-   R  =&gt;  S.
            P  =&gt;  Q   :-   R  =&gt;  S,  T  =&gt; U.
    </pre>
    The rules are written in a <em>Prolog</em>-like syntax. The turnstyle <kbd>:-</kbd>is pronounced "if". On its left is the conclusion or <em>consequent</em>. On the
    right is a premise or <em>antecedent</em>. The antecedent can be a conjunction, as
    in the second form above, and the comma <kbd>,</kbd> is pronounced "and". So the
    ...
    <code>T</code> can be replaced by <code>U</code>". Details of these rules are given
    in the next two sections.
    <p>
      Using the same notation, the relation <kbd>==&gt;</kbd> is defined by the three
      rules
    </p>
    <pre>          P  ==&gt;  Q     :-   P  =&gt;  Q.
            P Q  ==&gt;  P R   :-   Q  ==&gt;  R.
    ...
    The last two clauses allow rewriting in a context --- <code>P</code> on the
    left or <code>R</code> on the right.
    <p>
      The next rules for <code> ==&gt; </code> concern combinators. In the following,
      <code>C</code>$i$ is any combinator expecting at least $i$ quotation parameters.
    </p>
    <pre>        [P] C1  ==&gt;  [Q] C1   :-   P ==&gt; Q.
            [P] [R] C2  ==&gt;  [Q] [R] C2   :-   P  ==&gt; Q.
    ...
    <p>
      The final relation to be introduced is <kbd>==&gt;&gt;</kbd>, the reflexive transitive
      closure of <code>==&gt;</code>. It is defined by
    </p>
    <pre>        P  ==&gt;&gt;  P.
            P  ==&gt;&gt;  R   :-   P  ==&gt;  Q,  Q  ==&gt;&gt; R.
    </pre>
    <p>
      The simplest examples of rewriting axioms are those generated by <em>definition</em>s.
    ...
    <p>
      ...
      a special list which is modified by programs. The first general operator is <kbd>newstack</kbd>,
      ...
      <code>newstack</code> is in fact the <em>right zero</em> element for program concatenation.
    </p>
    ...
    that: The <kbd>stack</kbd> operator pushes onto the stack a list containing all the
    elements of the stack. The <kbd>unstack</kbd> operator expects a list on top of the
    stack and makes that the stack. The <code>unstack</code> operator undoes what the
    <code>stack</code> operator does, but the reverse is true only in special cases.
    <pre>        newstack stack  =&gt;  newstack [].
            [] unstack   =&gt;   newstack.
            newstack  L   =&gt;  [L]  reverse  unstack.
    </pre>
    In the last rule, <code>L</code> has to be a list of literals. Also, it should
    be noted that the stack is not always a sequence of values, it can also contain operators
    and combinators. So, strictly speaking the stack is always a quotation, and the <code>stack</code>operator pushes a quotation onto the stack, and the <code>unstack</code> operator
    ...
    An <em>operator</em> denotes a function which expects as argument one stack whose
    ...
    for operators on <em>simple type</em>s such as truth values, characters and integers.
    These are then followed by rules for operators on <em>aggregate type</em>s such as
    sets, strings and lists.
    <p>
      The unary operators <kbd>pop</kbd> and <kbd>dup</kbd> are defined on all stacks
      containing at least one element. In the rewrite rules to follow, let <code>a</code>  be any literal or a program whose net effect is to push exactly one value onto
      the stack.
    </p>
    <pre>        a  pop   =&gt;   id.
            a  dup   =&gt;   a  a.
    ...
    pushing two occurrences of the multiplication operator <code>*</code> onto the stack.
    <p>
      The binary operators <kbd>swap</kbd>, <kbd>popd</kbd>, and <kbd>dupd</kbd> are
      defined on all stacks containing at least two elements. Let <code>a</code> and
      <code>b</code> be any literals or equivalent programs.
    </p>
    <pre>        a  b  swap   =&gt;   b  a.
            a  b  popd   =&gt;   b.
            a  b  dupd   =&gt;   a  a  b.
    </pre>
    The ternary operators <kbd>swapd</kbd>, <kbd>rollup</kbd>, <kbd>rolldown</kbd>and <kbd>rotate</kbd> are defined on all stacks containing at least three elements.
    Let <code>a</code>, <code>b</code> and <code>c</code> be any literals or equivalent
    programs.
    ...
    <p>
      The ternary operator <kbd>choice</kbd> also expects three elements, but the third
      element has to be a truth value. Let <code>a</code> and <code>b</code> be any literals
      or equivalent.
    </p>
    <pre>        true   a  b  choice   =&gt;   a.
            false  a  b  choice   =&gt;   b.
    </pre>
    <p>
    ...
    <p>
      ...
      <kbd>succ</kbd>, <kbd>pred</kbd>, <kbd>abs</kbd> and <kbd>sign</kbd> for integer
      ...
      can also be applied to truth values. In what follows, let <code>i</code>, <code>j</code>  and <code>k</code> be any integers.
    </p>
    <pre>        i  succ   =&gt;   j.               ( j = i+1 )
            i  pred   =&gt;   j.               ( j = i-1 )
            i  abs    =&gt;   j.               ( j = abs(i) )
    ...
    <p>
      ...
      <kbd>max</kbd> and <kbd>min</kbd> for integers operands are as follows.
    </p>
    <pre>        i  j  +     =&gt;   k.             ( k = i+j )
            i  j  -     =&gt;   k.             ( k = i-j )
            i  j  *     =&gt;   k.             ( k = i*j )
    ...
    In the mixed case the type of the result <code>k</code> is the same as the type of
    the second parameter <code>i</code>.
    <p>
    ...
    <p>
      The type of truth values is one of the Boolean types. The <em>binary operator</em>s
      are <kbd>and</kbd>, <kbd>or</kbd> and <kbd>xor</kbd> (exclusive or). The <em>unary operator</em>  is <kbd>not</kbd>. Let <code>p</code> and <code>q</code> be truth values
      <code>true</code> or <code>false</code>.
    </p>
    <pre>        p  q  and   =&gt;   r.             ( r = p and q )
            p  q  or.   =&gt;   r              ( r = p or  q )
            p  q  xor.  =&gt;   r              ( r = p xor q )
            p  not      =&gt;   r.             ( r = not p )
    </pre>
    <p>
      ...
      <em>unary predicate</em>s
      <kbd>null</kbd>, <kbd>small</kbd>, <kbd>odd</kbd>, <kbd>even</kbd>, <kbd>positive</kbd>  and <kbd>negative</kbd> are defined for all numeric types:
    </p>
    <pre>        i  null       =&gt;   p.           ( p = (i=0) )
            i  small      =&gt;   p.          ( p = (i&lt;2) )
            i  odd        =&gt;   p.           ( p = odd(i) )
    ...
    <p>
      ...
      <kbd>=</kbd>, <kbd>!=</kbd>, <kbd>&lt;</kbd>, <kbd>&lt;=</kbd>, <kbd>&gt;</kbd>  and <kbd>&gt;=</kbd> have the obvious rewrite rules:
    </p>
    <pre>        i  j  =    =&gt;   p.              ( p = (i = j) )
            i  j  !=   =&gt;   p.              ( p = not(i = j) )
            i  j  &lt;    =&gt;   p.              ( p = (i &lt; j) )
    ...
    <p>
      The remainder of this section deals with <em>aggregate type</em>s: sets, strings
      and quotations, with lists as a special case. The <em>unary operator</em>s
      <kbd>first</kbd>, <kbd>second</kbd>, <kbd>third</kbd> and <kbd>rest</kbd> expect
    ...
    <p>
      Here <code>[a L]</code> is a non-empty list or quotation whose first member is
      <code>a</code> and whose rest is <code>[L]</code>. For strings an analogous notation
      can be used to obtain analogous rules. For example
    </p>
    <pre>        "cS"  first   =&gt;   'c.
            "cS"  rest    =&gt;   "S".
    </pre>
    Here "cS" denotes a non-empty string whose first character is <code>'c</code>and whose remaining characters are the string <code>"S"</code>.
    <p>
    ...
    Here <code>{a S}</code> denotes a non-empty set whose smallest member is <code>a</code>and whose other members are those of <code>{S}</code>.
    <p>
      The <em>binary operator</em>s <kbd>cons</kbd> and <kbd>swons</kbd> expect an aggregate
    ...
    <p>
      The <em>unary operator</em>s <kbd>uncons</kbd> and <kbd>unswons</kbd> also expect
    ...
    <p>
      The two <em>binary operator</em>s <kbd>at</kbd> and <kbd>of</kbd> are for <em>index</em>ing
      into aggregates. For list the rules might be written:
    </p>
    <pre>        [l1 l2 ... li ... ln]  i  at   =&gt;   li.
            i  [l1 l2 ... li ... ln]  of   =&gt;   li.
    </pre>
    ...
    is used. But the notation with the dots <code>...</code> is not satisfactory. Here
    is a better version for <code>at</code> applied to lists:
    <pre>        [a L]  1  at   =&gt;   a.
    ...
    <p>
      The unary operator <kbd>size</kbd> takes an aggregate and determines the number
      of elements:
    </p>
    <pre>           []  size  =&gt;   0.
    ...
    <p>
      The unary operator <kbd>reverse</kbd> can be applied to any aggregate but it is
      useful only for sequences:
    </p>
    <pre>           []  reverse  =&gt;  [].
            [a L]  reverse  =&gt;  [M a]   :-   L reverse  =&gt;  M.
    </pre>
    These rules for <code>reverse</code> are correct but inefficient since appending
    to the right to produce <code>[M a]</code> requires copying --- at least for the
    obvious implementation of lists. Most implementations would use an <em>accumulating parameter</em>to optimise the <code>reverse</code> operator. It is of some interest that this optimisation
    ...
    <p>
      The binary operator <kbd>concat</kbd> can be applied to two sequences which are
    ...
    <p>
      The operators <kbd>and</kbd>, <kbd>or</kbd>, <kbd>xor</kbd> and <kbd>not</kbd>  can be applied not only to truth values but also to values of the <em>set type</em>.
      The reduction rules look exactly as for the truth values, except that the operations
      have to be performed bitwise. So they compute the <em>intersection</em>, <em>union</em>,
      <em>symmetric difference</em> and <em>complement</em> with respect to the largest
      set.
    </p>
    <p>
      The two <em>unary predicate</em>s <kbd>null</kbd> and <kbd>small</kbd> can also
    ...
    <p>
      The two binary predicates <kbd>in</kbd> and <kbd>has</kbd> test aggregates for
      members.
    </p>
    <pre>        a  []  in   =&gt;   false.
            a  [a L]  in   =&gt;   true.
    ...
    <p>
      Most implementations of Joy will provide an operator for <kbd>sort</kbd>ing a sequence
      and a binary operator <kbd>merge</kbd> for combining two already sorted sequences.
    ...
    <p>
      Sometimes it is necessary to test a parameter for its type. The <em>unary predicate</em>s
      <kbd>logical</kbd>, <kbd>char</kbd>, <kbd>integer</kbd>,
      <kbd>set</kbd>, <kbd>string</kbd> and <kbd>list</kbd> are true if the parameter
    ...
    <p>
      ...
      lists on top of the stack and below that one further item. The <kbd>opcase</kbd>  operator matches the type of the item with the <code>first</code> members of the
      lists. When a match is found, the <code>rest</code> of that list is pushed onto
      the stack. If no match is found, then the last list is used as the default.
    </p>
    <pre>        123      [ [0 P] ['a Q] ["" R] ... ]  opcase   =&gt;   123     [P].
            'c       [ [0 P] ['a Q] ["" R] ... ]  opcase   =&gt;   'c      [Q].
            "Hello"  [ [0 P] ['a Q] ["" R] ... ]  opcase   =&gt;   "Hello" [R].
    </pre>
    <h1>Rules for combinators</h1>
    <p>
      The simplest <em>unary combinator</em>s are <kbd>i</kbd> and <kbd>x</kbd>, they
      require the top of the stack to be <em> one</em> quotation. Let <code>P</code>  be any program.
    </p>
    <pre>        [P]  i   =&gt;   P.
            [P]  x   =&gt;   [P]  P.
    </pre>
    <p>
      ...
      allow manipulation of the stack below the top few elements. Let <code>P</code>  be any program, let <code>a</code>, <code>b</code> and <code>c</code> be any literals
      or equivalent.
    </p>
    <pre>        a        [P]  dip   =&gt;   P  a.
            a  b     [P]  dip2  =&gt;   P  a  b.
    ...
    <p>
      Another unary combinator is <kbd>nullary</kbd>. Its rewrite rule has to be expressed
      conditionally. Let <code>L</code>, <code>M</code> and <code>P</code> be any programs.
    </p>
    <pre>        L  [P]  nullary   =&gt;   L  a    :-   L  P   =&gt;   M  a.
    </pre>
    ...
    <p>
      ...
      <kbd>app1</kbd>, <kbd>app2</kbd> and <kbd>app3</kbd>. Let <code>a</code>, <code>a'</code>,
      <code>b</code>, <code>b'</code>, <code>c</code> and <code>c'</code> be any literals
      or equivalent. Note that the primed versions are used as the result of applying
      <code>P</code> to the unprimed versions.
    </p>
    <pre>        L  a  [P]  app1   =&gt;   L  a'   :-
                L a P  =&gt;  M  a'.
            L  a  b  [P]  app2   =&gt;   L  a'  b'   :-
    ...
    There is even an <kbd>app4</kbd> combinator which applies <code>[P]</code>to four parameters <code>a</code>, <code>b</code>, <code>c</code> and <code>d</code>.
    <p>
      The <em>binary combinator</em>s expect two quotations on top of the stack. The
      <kbd>b</kbd> combinator expects two quotations <code>[P]</code> and <code>[Q]</code>,
      with <code>[Q]</code> on top.
    </p>
    <pre>        [P]  [Q]  b   =&gt;   P  Q.
    </pre>
    The <kbd>cleave</kbd> combinator also expects two quotations, and below that
    an item <code>a</code>.
    ...
    <p>
      ...
      of the most important is <kbd>ifte</kbd> which performs branching. Its third parameter
    ...
    <p>
      ...
      repeatedly executes the while-part and while that yields <code>true</code> it executes
    ...
    The ternary <kbd>tailrec</kbd> combinator for <em>tail recursion</em> also
    ...
    <p>
      ...
      The <kbd>linrec</kbd> combinator for <em>linear recursion</em> expects an if-part
      <code>[I]</code>, a then-part <code>[T]</code>, and two recursion parts <code>[R1]</code>  and <code>[R2]</code>.
    </p>
    <pre>    L [I] [T] [R1] [R2] linrec  =&gt;  L  T   :-
                L  I   =&gt;   M  true.
        L [I] [T] [R1] [R2] linrec  =&gt;  L R1 [I] [T] [R1] [R2] linrec R2   :-
                L  I   =&gt;   M  false.
    </pre>
    <p>
      The <kbd>binrec</kbd> combinator for <em>binary recursion</em> is similar, except
    ...
    <p>
      The <kbd>genrec</kbd> combinator for <em>general recursion</em> is also has an
    ...
    <p>
      ...
      Instead they use a list of quotations. The <kbd>cond</kbd> combinator is like
      the one in Lisp, it is a generalisation of the <code>ifte</code> combinator. It
      ...
      <code>true</code>, and then its corresponding then-part is executed. The last program
      in the list is the default which is executed if none of the if-parts yield <code>true</code>.
    </p>
    <pre>        L  [ [[I1] T1] REST ]  cond   =&gt;  L  T1   :-
                L  I1   =&gt;   M  true.
            L  [ [[I1] T1] REST ]  cond   =&gt;   L [ REST ] cond   :-
                L  I1   =&gt;   M  false.
    </pre>
    <p>
      The <kbd>condlinrec</kbd> combinator is similar, it expects a list of pairs or
      ...
      yields <code>true</code> selects its corresponding then-part or rec1-part for execution.
    ...
    <p>
      The <code>cleave</code> combinator also has a generalisation, The <kbd>construct</kbd>  combinator expects two parameters, a quotation and above that a list of quotations.
    ...
    <pre>        L [P] [..[Qi]..] construct =&gt;  L P ..qi..  :-  L Qi  =&gt;  M qi.
    </pre>
    <p>
    ...
    <p>
      The binary combinator <kbd>branch</kbd> expects a truth value below its two quotation
      parameters: The <kbd>branch</kbd> combinator resembles the
      <code>choice</code> operator and the <code>ifte</code> combinator. The truth value
      below the two quotations determines which of the two quotations will be executed.
      If the truth value is <code>true</code>, then the if-part, the second parameter,
      is executed, otherwise the then-part, the top parameter, is executed.
    </p>
    <pre>        true   [P]  [Q]  branch   =&gt;   P.
            false  [P]  [Q]  branch   =&gt;   Q.
    </pre>
    <p>
      The unary combinator <kbd>times</kbd> expects a numeric value below its quotation
      parameter: The <code>times</code> combinator executes its quotation parameter as
    ...
    <p>
      ...
      of the stack. The <kbd>infra</kbd> combinator expects a quotation <code>[P]</code>  which will be executed and below that another quotation which normally will be
      just a list <code>[M]</code>. The <code>infra</code> combinator temporarily discards
      the remainder of the stack and takes the quotation or list <code>[M]</code> to
      be the stack. It then executes the top quotation <code>[P]</code> which yields
      a result stack. This resulting stack is then pushed as a list <code>[N]</code>  onto the original stack replacing the original quotation or list. Hence any quotation
      can serve as a complex unary operation on other quotations or lists.
    </p>
    <pre>        L  [M]  [P]  infra  =&gt;  L  [N]   :-  [M]  unstack  P  =&gt;  N.
    </pre>
    <p>
      For linear recursion over <em>numeric type</em>s the if-part often is <code>[null]</code>  and the first recursion part is <code>[dup pred]</code>. The <kbd>primrec</kbd>  combinator has this built in. For integers the rewrite rules are:
    </p>
    <pre>        0  [T]  [R2]  primrec   =&gt;   pop  T.
            i  [T]  [R2]  primrec   =&gt;   i  dup pred  [T]  [R2]  primrec  R2.
    </pre>
    The <code>primrec</code> combinator can also be used for aggregates. The implicit
    if-part is again <code>[null]</code>, and the implicit first recursion part is <code>[rest]</code>.
    ...
    <p>
      ...
      <kbd>map</kbd>, <kbd>filter</kbd> and <kbd>split</kbd> all expect an aggregate
    ...
    to follow. For <kbd>map</kbd> operating on lists the rewrite rule is:
    <pre>        []  [P]  map   =&gt;   [].
            K  [a L]  [P]  map   =&gt;   K  [b M]   :-
                K  a  P   =&gt;   K  b,   K  [L]  [P]  map   =&gt;   K  M.
    </pre>
    The <kbd>filter</kbd> combinator expects a predicate as its quotation parameter.
    <pre>        []  [P]  filter   =&gt;   [].
    ...
    The <kbd>split</kbd> combinator is like <code>filter</code> except that it
    produces two lists. The first list is just like the one from <code>filter</code>,
    the second list is the list of those elements which did not pass the predicate test
    <code>[P]</code> and hence are not members of the first list.
    <pre>        []  [P]  split   =&gt;   [].
    ...
    <p>
      The unary combinator <kbd>fold</kbd> expects a quotation which computes a binary
    ...
    <p>
      The two <em>unary combinator</em>s <kbd>some</kbd> and <kbd>all</kbd> expect an
      aggregate below their quotation parameter. The quotation must be a predicate, yielding
      a truth value. The <kbd>some</kbd> combinator returns <code>true</code> if some
      members of the aggregate pass the test of the quotation, otherwise it returns <code>false</code>.
      The <kbd>all</kbd> combinator returns <code>true</code> if all members of the aggregate
      pass the test of the quotation, otherwise it returns <code>false</code>. For empty
      aggregates <code>some</code> returns <code>false</code> and <code>all</code> returns
      <code>true</code>. The rules for <code>some</code> are:
    </p>
    <pre>        []  [P]  some   =&gt;   false.
            L  [a A]  [P]  some   =&gt;   L  true   :-
                L  a  P  =&gt;  M  true.
    ...
    <p>
      The unary combinator <kbd>zipwith</kbd> expects two aggregates and above that a
    ...
    <code>20</code>, and it is independent of the order in which the reductions have
    ...
    <p>
      Scan the expression from left to right until a <em>redex</em> is found, an expression
    ...
    The strategy requires skipping the <code>3</code>, <code>2</code> and <code>6</code>and only then replacing <code>8 6 -</code> by <code>2</code>. The next step requires
    skipping <code>3</code> and <code>2</code> and only then replacing <code>6 2 /</code>by <code>3</code>. The next step requires skipping <code>3</code> and only then replacing
    <code>2 3 +</code> by <code>5</code>. The final step requires no skipping,
    <code>3 5 *</code> is replaced by <code>15</code>. All this skipping is of course
    inefficient.
    <p>
      A better strategy would apply the next operator at the point of the most recent change,
      if that is possible. An obvious way to do this is to use a <em>stack of values</em>  for intermediate results. As the expression is being processed, operands such as
    ...
    <p>
      But this now raises the question whether the stack is <em> just</em> an optimisation
    ...
    <p>
      It will help to review the stack based semantics of Joy: The <em>literal</em>s
      ...
      stack except that a single item has been pushed on top. The <em>operator</em>s
      ...
      of applying some operation. Likewise, the <em>combinator</em>s denote unary functions
    ...
    <p>
      ...
      without a stack: Joy programs denote unary functions taking one <em>program</em>  as arguments and giving one <em>program</em> as value. The <em>literal</em>s denote
      <em> append</em> operations; the program returned as value is like the program
      given as argument, except that it has the literal appended to it. The <em>operator</em>s
      denote <em> replacement</em> operations, the last few items in the argument program
      have to be replaced by the result of applying the operator. Similarly the <em>combinator</em>s
    ...
    The first results in the one operator <code>*</code> being pushed. The second
    results in two operators and one combinator to be pushed. Such situations are required
    only rarely. But the possibility is needed, for example for a Joy interpreter <kbd>joy</kbd>written in Joy itself. Such an interpreter is described in another paper.
    <h1>Quotation revisited</h1>
    <p>
      It was mention in section 3 that for <em>quotation</em>s there is no rewrite rule
      of the form
    </p>
    <pre>        [P]  ==&gt;  [Q]   :-   P  ==&gt;  Q.
    </pre>
    ...
    but only in those where the quotation is guaranteed to be undone by a <em>dequoting</em>operation, by a combinator. In other words, quotation is an <em>intensional constructor</em>.
    <p>
      There is a simple way out of this, and it is to treat quotations of programs to be
      very different from lists. Notice that the absurdity comes from taking the <code>second</code>  element of the quotations <code>[42 dup]</code> and <code>[42 42]</code>. If it
      were forbidden to treat quoted programs as data structures, then the fatal inference
      would be blocked. In detail, such a treatment would look like this: If <code>P</code>  is a program, then <code>(P)</code> is its quotation, now written inside round
      parentheses. Also, <code>[P]</code> is its list, as before written inside square
      brackets. Both <code>(P)</code> and <code>[P]</code> can be pushed onto the stack,
      can be <code>swap</code>ped, <code>dup</code>licated and <code>pop</code>ped, can
      be inserted into lists and later extracted. But only <code>(P)</code> can be used
      as a parameter for combinators, and it cannot treated as a list. On the other hand,
      <code>[P]</code> cannot be used as a parameter for combinators, but it can be treated
    ...
    of the stack is, say <code>7</code>, then the <code>cons</code> operation produces
    <code>(7 +)</code> to be used by <code>map</code>. The prohibition would rule out
    <em>parameterisation</em>. In general, the prohibition would rule out using <em>constructed program</em>s
    as parameters to combinators.
    <p>
      It is possible to make a less drastic compromise: As before, quotations <code>(P)</code>  serve as parameters to combinators, but they can also be built up by list operations
      such as <code>concat</code>enation or <code>cons</code>ing further items into their
      front. This would allow parameterisation as in the <code>map</code> example above.
    ...
    <h1>Rewriting for Joy types</h1>
    <p>
      The rewriting system described up to here concerned values. It is also possible to
      give a rewriting system for Joy <em>type expression</em>s. We shall need constant
      and variable symbols for these types. The following will be used as <em>type constant</em>s:
      ...
      <code>Lst</code> for possibly heterogeneous lists. For lists whose member are all
      of the same type, say <code>Int</code>, the notation <code>[Int]</code> will be
      used. As variables we use
      <code>T</code>, <code>T1</code>, <code>T2</code> and so on. So <code>[T]</code>  is the type of lists whose members are all of the type <code>T</code>.
    </p>
    <p>
      The following is a sample of one style of rules for operators:
    </p>
    <pre>        T1   T2  swap   =&gt;   T2  T1.
            Int  Int  +     =&gt;   Int.
            Str      size   =&gt;   Int.
    </pre>
    ...
    <p>
      Literals have <em>atomic type</em>s, operators and combinators have <em>compound type</em>s.
      ...
      is new and has no counterpart program constructor. It uses the infix symbol <kbd>-&gt;</kbd>  to combine two types into a new one. If <code> T </code> is a type, then so is
      its quotation <code> [T] </code>. If <code> T1 </code> and <code> T2 </code> are
      types, then so are their concatenation
      <code> (T1 T2) </code> and their cancellation <code> T1-&gt;T2 </code>. If <code> P1 </code>  and <code> P2 </code> are programs of types
      <code> T1 </code> and <code> T2 </code>, then the type of their concatenation <code> (P1 P2) </code>  is the concatenation <code> (T1 T2) </code> of their types. Cancellation satisfies
      the law
    </p>
    <pre>                T1  T1-&gt;T2   =&gt;   T2
    </pre>
    ...
    on the convention that the cancellation operator <code>\</code> is taken to be right
    associative.
    <p>
      The three rules given above should now be rewritten in this style:
    </p>
    <pre>        swap   =&gt;     (T1 T2) -&gt; (T2 T1).
            +      =&gt;   (Int Int) -&gt; Int.
            size   =&gt;         Str -&gt; Int.
    </pre>
    <p>
    ...
    Consider now the program <code> P </code> below. Its type is given by the
    concatenation of the types of its parts, in the line just below. All the types here
    are built from the atomic type <code>  Int  </code> of integers. By four applications
    of cancellation the type in line 1 is simplified to the type <code> Int </code> in
    line 4.
    <pre>P:    2      3      +                dup              *
    ...
    <p>
      For combinators only only a few examples will be given here, for <kbd>i</kbd> and
      <kbd>map</kbd>.
    </p>
    <pre>        i   =&gt;   [T] -&gt; T.
            map   =&gt;   ([T1 -&gt; T2]) -&gt; ([T1] -&gt; [T2]).
    ...
    <p>
      The formalism used in this section is that of <em>categorial grammar</em>s. These
    ...
    which is <em> not</em> a rewriting rule but a <em> statement</em> which says
    that program <code>P</code> is of type <code>T</code>. The basic type statements
    ...
    <kbd>=&gt;</kbd> a single conditional rule is needed which converts the semantic
    predicate <kbd>:</kbd> into the syntactic <code>=&gt;</code>, as follows.
    <pre>        X =&gt; T   :-   X : T.
    </pre>
    <p>
