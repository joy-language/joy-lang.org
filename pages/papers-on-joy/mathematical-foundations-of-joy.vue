<template>
  <article>
    ...
    <p>
      <em>Keywords:</em> functional programming, syntactic and semantic monoids, function
    ...
    <p>
      ...
      and the value are <em>state</em>s consisting of at least three components. The
      principal component is a
      <em>stack</em>, and the other components are not needed here. Much of the detail
    ...
    <p>
      ...
      are provided by the
      <em>combinator</em>s of Joy. These denote functions which behave like higher order
      functions in other languages.
    </p>
    <p>
      The above may be summarised as follows: Let <code>P</code>,
      <code>Q1</code>, <code>Q2</code> and <code>R</code> be programs, and let <code>C</code>  be a combinator. Then this principle holds:
    </p>
    <pre>        IF          Q1      ==      Q2
            THEN     P  Q1  R   ==   P  Q2  R
            AND        [Q1] C   ==     [Q2] C
    ...
    The design of Joy was motivated by <a href="refs.html#{Quine71}">{Quine71}</a> and
    <a href="refs.html#{Backus78}">{Backus78}</a> who in quite different fields examine
    ...
    in the language <em>FP</em>. The language Joy offers a very different solution to
    the same goals. Paulson <a href="refs.html#{Paulson92}">{Paulson92}</a> remarked
    ...
    <p>
      ...
      structures are so similar. In particular, the two structures are <em>monoid</em>s
      and the meaning function which maps syntax into semantics is a <em>homomorphism</em>.
    </p>
    <p>
      ...
      m2 ...}</code> including a special element <code>m</code>, and a binary operation,
      written, say, as infix period <code>"."</code>. The special element has to be a
      left and right <em>unit element</em> for the binary operation, and the binary operation
      has to be
    ...
    For example, these are monoids: the integers with <code>0</code> as the unit
    element and addition as the binary operation, or the integers with <code>1</code>as the unit element and multiplication as the binary operation. Two examples from
    ...
    and concatenation as the binary operation. Concatenation is <em> not</em> commutative.
    <p>
    ...
    <p>
      Unit elements should be distinguished from <em>zero element</em>s, which behave
      the way the number <code>0</code> interacts with multiplication: a product containing
    ...
    <pre>        h(m)  =  n                      h(x . y)  =  h(x) . h(y)
    </pre>
    In the second equation, the binary operation on the left is that of
    <code>M</code>, and the one on the right is that of <code>N</code>. One example is
    the logarithm function which is a homomorphism from the multiplicative monoid onto
    the additive monoid. Another example of a homomorphism is the <kbd>size</kbd> (or
    ...
    <code>++</code> are used for the empty list and for concatenation.) Other examples
    ...
    The <em>syntax</em> of Joy programs is very simple: the basic building blocks are
    ...
    <p>
      Now to the <em>semantics</em>. In the introduction it was said that Joy uses postfix
    ...
    <p>
      The operation of <em>function composition</em> is associative and there is a left
      and right unit element, the <em>identity
    ...
    If the <em> programs</em> <code>P</code> and <code>Q</code> denote the same function,
    then the <em> functions</em> <code>P</code> and
    <code>Q</code> are <em> identical</em>. Two functions are identical if for all values
    in the intersection of their domains they yield the same value. This will be written
    <pre>        P   ==   Q
    </pre>
    The symbol <code>==</code> will be used to denote the identity of Joy functions.
    The symbol does not belong to the language Joy but to its metalanguage. The <em>identity relation</em>between functions is clearly <em>reflexive</em>, <em>symmetric</em> and
    <em>transitive</em>. Furthermore, identicals are indiscernible in larger contexts
    such as compositions. Hence <em>substitution</em> of identicals can be used as a
    rule of inference:
    <pre>        IF             Q1      ==      Q2
            THEN        P  Q1  R   ==   P  Q2  R
    </pre>
    ...
    The notation can be used to express what <em> look</em> like identities of
    numbers; for example
    <pre>        2  3  +   ==   5
    </pre>
    expresses that the composition of the three <em> functions</em> on the left
    is identical with the one <em> function</em> on the right. On the left, the first
    two functions push the <em> numbers</em> 2 and 3 onto the stack, and the third replaces
    them by their sum. On the right, the function pushes the <em> number</em> 5. The
    ...
    <p>
      ...
      <code>2</code>, <code>3</code>, <code>+</code> and
      <code>5</code> denotes a function which takes a stack as argument and yields a
      stack as value. The three <em> numerals</em> <code>2</code>,
      <code>3</code> and <code>5</code> denote <em> functions</em> which are defined
      for all argument stacks. They yield as values other stacks which are like the argument
      stacks except that a new <em> number</em>, 2, 3 and 5 has been pushed on top.
    </p>
    <p>
      ...
      It follows that the above equation does <em> not</em> express the identity of numbers
      but the identity of <em> functions</em>.
    </p>
    <p>
      The associativity of composition has as a consequence about
      <em>currying</em>: that there is no difference between standard and curried operators.
      Consider the example
    </p>
    <pre>        (2  3)  +   ==   2  (3  +)
    </pre>
    On the left the <code>+</code> takes two parameters supplied by
    <code>(2 3)</code>. On the right <code>+</code> is given one parameter, <code>3</code>.
    The resulting function <code>(3 +)</code> expects one parameter to which it will
    add <code>3</code>. Because of associativity the two sides are identical and hence
    no parentheses are required.
    <p>
      Let <code>P</code> be a program which pushes <tt>m</tt> values onto the stack.
      Let <code>Q</code> be a program which expects <tt>n</tt> values on the stack, <tt>m</tt>  &lt;= <tt>n</tt>. Now consider their concatenation <code>P
    ...
    <p>
      ...
      <code>0</code> has been a rather recent innovation. The symbol enables one to say
      more than just that <code>0</code> is a unit element for addition. Similarly, in
    ...
    <p>
      In arithmetic <code>0</code> and <code>1</code> are unit elements for addition
      and multiplication, so adding <code>0</code> or multiplying by <code>1</code> have
      ...
      <kbd>swap</kbd>ping the top two elements twice has no net effect, and that duplicating
      the top element and then <kbd>pop</kbd>ping off the duplicate has no net effect.
    </p>
    <p>
      ...
      a number - in either order - produces no net effect. In the <em>algebra of Joy</em>  these are expressed by the following:
    </p>
    <pre>        0  +   ==   id                  1  *   ==   id
    ...
    <p>
      ...
      It is appropriate to remark here that there is also a <em>left zero element</em>  and there is a <em>right zero
      element</em>. Two such elements <code>l</code> and <code>r</code> satisfy the following
      for all programs <code>P</code>:
    </p>
    <pre>        l  P   ==   l                   P  r   ==   r
    </pre>
    Since function composition is not commutative, the two zero elements are not
    identical. In Joy the left zero <code>l</code> is the
    <kbd>abort</kbd> operator, it ignores any program following it. The right zero <code>r</code>is the <kbd>clearstack</kbd> operator, it empties the stack and hence ignores any
    ...
    program</em> or <em>quotation</em>. The length or <code>size</code> of the quotation
    <code>[5]</code> is <code>1</code>, and the size of the quotation <code>[2 3 +]</code>is <code>3</code>. However, as noted earlier, the two programs inside the brackets
    ...
    <p>
      ...
      but as an active program. In Joy such treatment is due to <em>combinator</em>s
      which in effect dequote one or more of their parameters.
    </p>
    <p>
      The <kbd>i</kbd> combinator expects a quoted program on top of the stack. It pops
      that program and executes it. So, if the quoted program <code>[P]</code> has just
    ...
    is like the argument stack but has the number <code>5</code> pushed on top.
    <pre>         2  3  +                         5
            [2  3  +] i                     [5] i
    </pre>
    If the program <code>P</code> computes the identity function, then the effect
    of applying the <code>i</code> combinator is that of the identity function:
    <pre>        [id]  i   ==   id               []  i   ==   id
    </pre>
    Another law is this:
    <pre>        i  ==  []  cons  i  i
    ...
    <p>
      Two programs <code>P</code> and <code>Q</code> may look very different - for example,
    ...
    Suppose now that a quoted program, <code>[P]</code>, is on top of the stack.
    It could then be executed with the <code>i</code> combinator. But it could also be
    manipulated as a passive data structure first. For example, one could push the quotation
    <code>[i]</code> and then use the <kbd>cons</kbd> operator to insert <code>[P]</code>into
    <code>[i]</code> to give <code>[[P] i]</code>. What happens if this is executed by
    the <code>i</code> combinator? The internal
    <code>[P]</code> quote is pushed, and then the internal <code>i</code> combinator
    ...
    quoted program <code>[P]</code>. But it may help to spell out a consequence:
    <pre>        [P]  [i]  cons  i   ==   [[P] i]  i   ==   [P]  i   ==   P
    </pre>
    <p>
      The <code>i</code> combinator is only one of many. Another is the
      <kbd>b</kbd> combinator which expects two quoted programs on top of the stack.
      ...
      <code>[P]</code> and <code>[Q]</code> have just been pushed onto the stack, the
      <code>b</code> combinator will execute them in the order in which they have been
      pushed:
    </p>
    <pre>        [P]  [Q]  b   ==   P  Q
    </pre>
    It follows that the <code>b</code> combinator actually dequotes both of its
    ...
    <code>b</code> combinator is the identity function. If either of the two programs
    ...
    <p>
      ...
      and, more generally, quotations can be concatenated with the <kbd>concat</kbd>  operator. If <code>[P]</code> and
      <code>[Q]</code> have just been pushed, then they can be concatenated to become
      <code>[P Q]</code>. The resultant concatenation can be executed by the <code>i</code>  combinator. The net effect is that of executing the two programs, and that is also
      achieved by applying the
      <code>b</code> combinator:
    </p>
    <pre>        [P]  [Q]  concat  i   ==   P  Q   ==   [P]  [Q]  b
    </pre>
    But the two quoted programs do not have to be pushed immediately before the
    concatenation or the application of the <code>b</code> combinator. Instead they could
    have been constructed from smaller parts or extracted from some larger quotation.
    Hence the more general law:
    <pre>        concat  i   ==   b
    </pre>
    The equation could be reversed, hence the <code>b</code> combinator could
    be <em> defined</em> in terms of the <code>i</code> combinator. The <em> names</em><code>i</code> and <code>b</code> of the two combinators have been chosen because
    of their similarity to the <em>I combinator</em> and <em>B combinator</em> in <em>combinatory
    logic</em>. The standard text is <a href="refs.html#{Curry58}">{Curry58}</a>, but
    good expositions are to be found in many other books, for example <a href="refs.html#{Burge75}">{Burge75}</a>.
    ...
    <p>
      ...
      top. That is what the <kbd>dip</kbd> combinator is for. It is behaves like the
      <code>i</code> combinator by executing one quotation on top of the stack, except
      that it leaves the item just below the quotation unchanged. In detail, it expects
      a program
      <code>[P]</code> and below that another item <code>X</code>. It pops both, saves
      <code>X</code>, executes <code>P</code> and then restores
      <code>X</code>.
    </p>
    <p>
    ...
    <code>dip</code> combinator is to compute the identity function:
    <pre>        [id]  dip   ==   id             []  dip   ==   id
    </pre>
    Suppose a program <code>[P]</code> is on top of the stack, and it is first
    duplicated and then the copy executed with <code>dip</code> just below the original
    <code>[P]</code>. Now the original has been restored, but suppose it is now popped
    explicitly. The net effect was the same as executing just the original <code>[P]</code>with the
    <code>i</code> combinator:
    <pre>        i   ==   dup  dip  pop
    </pre>
    Suppose that there are two programs <code>[P]</code> and
    <code>[Q]</code> on top of the stack, with <code>[Q]</code> on top. It is required
    to execute <code>[P]</code> while saving
    <code>[Q]</code> above. One way to do that is this: First push
    <code>[i]</code>. Now <code>[Q]</code> is the second element. Executing <code>dip</code>will save <code>[Q]</code> and execute
    <code>[i]</code> on the stack which now has <code>[P]</code> on the top. That amounts
    to executing <code>[P]</code>, and after that
    <code>[Q]</code> is restored.
    <p>
      Suppose further that it is now required to execute <code>[Q]</code>, and that is
      easily done with the <code>i</code> combinator. The net effect of all this is the
      same as executing first <code>[P]</code> and then <code>[Q]</code>, which could
      have been done with the
      <code>b</code> combinator. Hence
    </p>
    <pre>        b   ==   [i]  dip  i
    </pre>
    The last two equations show that the <code>dip</code> combinator could be
    used to <em> define</em> both the <code>i</code> combinator and the <code>b</code>combinator. The reverse is not possible.
    <p>
      The last two equations also serve to illustrate algebraic manipulation of Joy programs.
      In the last equation the <code>i</code> combinator occurs twice, once quoted
      and once unquoted. Both occurrences can be replaced in accordance with the previous
      equation, and this yields
    </p>
    <pre>        b   ==   [dup dip pop]  dip  dup  dip  pop
    </pre>
    ...
    of dequotation, and in this case such a context is given by the <code>dip</code>combinator.
    <p>
      Again suppose that there are two quoted programs <code>[P]</code> and
      <code>[Q]</code> on the stack. If the <code>dip</code> combinator is executed next,
      it will cause the topmost quotation <code>[Q]</code> to be executed while saving
      and later restoring <code>[P]</code> below. Suppose that the <code>i</code> combinator
      is executed next, this will cause the restored <code>[P]</code> to be executed.
      So the net effect of the two combinators is to execute first <code>P</code> and
      then
      <code>Q</code>. That same effect could have been achieved by first swapping <code>[P]</code>  and <code>[Q]</code> around, so that
      <code>[P]</code> is on top, and then executing the <code>b</code> combinator. This
    ...
    <code>b</code> combinator and the <code>i</code> combinator are applied appropriately.
    For the left side this is easy:
    <pre>        [P]  [Q]  b  [R]  i   ==   [P]  [Q]  [R]  [b] dip i
    </pre>
    For the right side it is a little harder since the <code>i</code> combinator
    has to be applied to <code>[P]</code> which is obscured not by one but two other
    quotations. The <code>dip</code> combinator has to be used on itself in this case,
    as follows:
    ...
    It is possible to cancel the final <code>pop</code> on both sides, but it
    is not possible to cancel the prefinal <code>dip</code> on both sides. This unlikely
    law also expresses the associativity of function composition. But the most elegant
    way of expressing the associativity is by using a variant of the <code>dip</code>combinator, called
    ...
    (
    <a href="refs.html#{Henson87}">{Henson87}</a> criticises presentations of FP-systems,
    originally due to
    <a href="refs.html#{Backus78}">{Backus78}</a> in that they give no law to this effect
    although they use it in proofs.)
    <p>
      The combination of the <code>dip</code> combinator immediately followed by the
      <code>i</code> combinator is sometimes useful for arranging the top few elements
      on the stack in a form that is suitable for executing a quoted program <code>[P]</code>  that is at the top of the stack.
    </p>
    <p>
      This is how it is done: first another quoted program
      <code>[Q]</code> is pushed, and executed using the <code>dip</code> combinator.
      This will save and restore the <code>[P]</code>, but arrange the stack in accordance
      with <code>[Q]</code>. Then the restored <code>[P]</code> is executed by the <code>i</code>  combinator. Depending on the <code>[Q]</code> that is chosen, the three part combination
      of <code>[Q]</code>, the <code>dip</code> combinator and the <code>i</code> combinator
      will prepare the stack for the execution of <code>[P]</code>.
    </p>
    <p>
      Since such a combination still requires the <code>[P]</code> on the stack, any
      such combination has the effect of a combinator. The following illustrate some
      simple choices of <code>[Q]</code> that are sometimes useful. The names of these
      combinators have been chosen because of their similarity to the <em>K combinator</em>,
      <em>W
    ...
    Suppose that there is a quoted program <code>[P]</code> on top of the stack. This
    could now be executed by some combinator, say
    <code>C</code>. Alternatively, one could push the quotation
    <code>[C]</code> and then use the <code>cons</code> operator to insert the earlier
    <code>[P]</code> into the later quotation, and this produces <code>[[P] C]</code>.
    This of course may be executed by the
    <code>i</code> combinator. When that happens the inner
    <code>[P]</code> is pushed, thus partly undoing the <code>cons</code> operation.
    But then <code>C</code> will be executed. The net effect is the same as the earlier
    alternative. So we have: For all operators or combinators <code>C</code>
    <pre>        [C]  cons  i   ==   C
    </pre>
    It should be remarked that this theorem also holds for operators, say
    <code>O</code>, instead of combinators <code>C</code>.
    <p>
      Again suppose that there is a quoted program <code>[P]</code> on top of the stack.
      It could be executed by some combinator <code>C</code>, or one could do this: push
      the quotation <code>[i]</code>,
      <code>cons</code> the earlier <code>[P]</code> into that and now execute <code>C</code>.
      The <code>cons</code> operation produced
      <code>[[P] i]</code> and when this is executed by <code>C</code>, the inner <code>[P]</code>  is pushed partly undoing the <code>cons</code>. Then the <code>i</code> combinator
      actually executes this. The net effect is that of just executing <code>C</code>.
      Hence for all combinators <code>C</code>
    </p>
    <pre>        [i]  cons  C   ==   C
    </pre>
    <p>
    ...
    <pre>        [i]  cons  C   ==   [C]  cons  i
    </pre>
    <p>
      So far we have only encountered one combinator which takes two quoted parameters
      - the <code>b</code> combinator. But Joy has a large number of combinators which
    ...
    by allowing three <code>cons</code> operations to occur.
    <p>
      Finally, the second law generalises to all parameters of a combinator: any one parameter
      <code>[P]</code> can be replaced by <code>[[P]
      i]</code>. The replacement can of course be constructed by
      <code>cons</code>ing <code>[P]</code> into <code>[i]</code>. That of course may
      be done for all quotation parameters. If there is just the one parameter <code>[P]</code>,
      then <code>cons</code>ing it into
      <code>[i]</code> to produce <code>[[P] i]</code> is easy enough, as in the second
      law.
    </p>
    <p>
      If there are two parameters <code>[P]</code> and <code>[Q]</code> it already becomes
      tedious to change them to <code>[[P] i]</code> and
      <code>[[Q] i]</code>. If there are three or more quotation parameters, then the
      program to produce the three changes could be rather obscure.
    </p>
    <p>
      Joy has a combinator which can use a function to <kbd>map</kbd> the elements of
      ...
      number of one, two, three and so on elements from the stack. These are the <kbd>app1</kbd>  combinator, the <kbd>app2</kbd> combinator, the <kbd>app3</kbd> combinator and
      so on. These are just the right combinators to produce the changes required for
      the parameters of a combinator. The following laws hold for combinators <code>C1</code>,
      <code>C2</code> and <code>C3</code> requiring one, two or three quotation parameters:
    </p>
    <pre>        [[i] cons]  app1  C1   ==   C1
            [[i] cons]  app2  C2   ==   C2
            [[i] cons]  app3  C3   ==   C3
    </pre>
    ...
    Computationally it is of course pointless to replace a quotation such as <code>[P]</code>by <code>[[P] i]</code> if the quotations are being used as parameters for a combinator.
    But the replacements are invaluable in a Joy interpreter written in Joy. This interpreter
    is essentially a complex combinator, appropriately called <kbd>joy</kbd>, and it
    has to behave just like the <code>i</code> combinator. In the definition of the <code>joy</code>combinator, the implementation of all combinators uses the above mapping combinators
    but with
    ...
    <pre>    LET  factorial(n)  =  if n = 0 then 1 else n * factorial(n - 1)
         IN  factorial(5)
    </pre>
    The call in the second line should return <code>120</code>. Joy has a number
    of ways of doing essentially the same computation without introducing the <em> name</em><code>factorial</code> and without introducing the <em> name</em> of the formal parameter
    <code>n</code>. Several of these ways are still modelled on the recursive definition
    and have approximately the same length. Two of them are based on the fact that the
    definition has the pattern of <em>linear recursion</em>, indeed <em>primitive recursion</em>.
    As in all languages the use of
    <em>accumulating parameter</em>s can avoid the recursion altogether, but that is
    not the point here.
    <p>
      The humble <code>i</code> and <code>dip</code> combinators were certainly not designed
      ...
      <code>i</code> combinator to execute the duplicate. The <code>dip</code> combinator
      can be used to access the stack below the quoted body. The only other combinator
      needed is the
      <code>ifte</code> combinator which achieves the same kind of two-way branching
      as the <code>if-then-else</code> in the conventional definition above.
    </p>
    <p>
      This is the program:
    </p>
    ...
    <p>
      ...
      the actual parameter <code>5</code> onto the stack. Then the long quotation extending
      ...
      copy is executed by the <code>i</code> combinator. This execution has the effect
      of pushing the two short quotations in lines 2 and 3 and also the longer quotation
      in lines 4 to 6. So at this point the stack contains the parameter <code>5</code>  and above that four quotations.
    </p>
    <p>
      But now the <code>ifte</code> combinator in line 7 executes. It pops the last three
    ...
    <p>
      The comparison will yield a truth value which the <code>ifte</code> combinator
    ...
    <p>
      ...
      replaced by <code>1</code>, the factorial of <code>0</code>. On the other hand,
    ...
    <p>
      ...
      be simply executed by the <code>i</code> combinator, but first the body is duplicated
      in line 5 and then the duplicate is executed by the <code>i</code> combinator.
    ...
    <p>
      ...
      <code>i</code> combinator in lines 5 or 8 triggered this execution of the body.
      Ultimately the execution of the <code>i</code> combinator in line 8 will have completed,
      and at this point the parameter
      <code>5</code> from line 1 will have been replaced by its factorial
      <code>120</code> as required.
    </p>
    <p>
      Two short comments are in order: Firstly, the <em> description</em> of the program
      ...
      functions. Secondly, the program can be written using only the <code>dip</code>  combinator and the <code>ifte</code> combinator by substituting <code>dup dip
      pop</code> for the two calls of the <code>i</code> combinator in lines 5 and 8.
    </p>
    <p>
      Of course this program is a <em> tour de force</em>, it is ugly and inefficient.
      ...
      <kbd>primrec</kbd> combinator for primitive recursion. The entire program then
      is
    </p>
    <pre>        5  [1]  [*]  primrec
    </pre>
    ...
    The first <em> seems</em> to express the identity of numbers. The second <em> seems</em>to express the identity of functions which both double a given number which they
    expect on the stack. The third <em>
    ...
    <p>
      The mathematical discipline of <em>category theory</em> deals with functions of
      ...
      and to the "Combinatory Abstract Machine" <em>CAM</em>, see for example <a href="refs.html#{Poigne92}">{Poigne92}</a>.
    </p>
    <p>
      The paper
      <a href="/overview-of-joy">
